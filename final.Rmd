...
trash <- request_311%>%
  filter(grepl("RF-COLLT",SERVICE_CODE))%>%
  dplyr::select(Y = LATITUDE, X = LONGITUDE) %>%
  st_as_sf(coords = c("X", "Y"), crs = 4326, agr = "constant") %>%
  st_transform('ESRI:102723') %>%
  st_transform(st_crs(cin_fishnet))%>%
...
grid.arrange(ncol=2,
             ggplot() + 
               ...
               labs(title= "Observed opioid overdose Cases in 2016",
             ...
             ggplot() + 
               ...
               scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
               labs(title = "Density of Observed opioid overdose Cases in 2016",
...
vars_net <- 
  rbind(abandon_cars, dead_animals,pothole,trash) %>%
...
reg.vars <- c("abandon_cars.nn" , "dead_animals.nn","pothole.nn", "trash.nn")
reg.ss.vars <- c("abandon_cars.nn" , "dead_animals.nn","pothole.nn", "trash.nn", "opioid.isSig" ,"opioid.isSig.dist")
...
rbind(burg_KDE_sf, burg_risk_sf) %>%
  ...
  ggplot() +
    ...
    labs(title="Comparison of Kernel Density and Risk Predictions",
         subtitle="2017 weapon violation risk predictions; 2018 weapon violation") +
    mapTheme(title_size = 14)
```
...
rbind(burg_KDE_sf, burg_risk_sf) %>%
  ...
  mutate(Rate_of_test_set_opioid = countopioid / sum(countopioid)) %>%
    ggplot(aes(Risk_Category,Rate_of_test_set_opioid)) +
      ...
      scale_fill_viridis(discrete = TRUE) +
      labs(title = "Risk prediction vs. Kernel density, 2018 weapon violation") +
